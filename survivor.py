...
class SurvivorStateAttacking(State):
    ...
    def shoot_zombie(self): 
        ...
        if zombie is not None:
            bullet = Bullet(self.survivor.world, self.survivor.bullet_image, 
                self.survivor.blood_image)
...
class SurvivorStateEvading(State):
    ...
    def run_away(self):
        ...
        zombie = self.survivor.world.get_close_entity_in_state("zombie", 
            ["wandering", "seeking"], self.survivor.location, 115)
        # If you can't find a non-feeding zombie to run from, see if a 
        #feeding one is close.
        if zombie is None:
            zombie = self.survivor.world.get_close_entity("zombie", 
